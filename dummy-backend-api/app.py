...
def init_db():
    ...
    cur.execute("INSERT INTO users (email, password, role) VALUES ('admin@example.com', 'admin123', 'admin')")
    cur.execute("INSERT INTO users (email, password, role) VALUES ('user@example.com', 'user123', 'user')")
    conn.commit()
    conn.close()
...
def login():
    ...
    app.logger.warning(f"Login request email={email} token={token} api_key={api_key}")

    # ISSUE: no authentication logic / no validation
    return jsonify({
        "status": "ok",
        "email": email,
        "token": token,
...
def admin_users():
    ...
    return jsonify([
        {"id": r[0], "email": r[1], "password": r[2], "role": r[3]}
        for r in rows
    ])
...
def upload():
    ...
    save_path = os.path.join("data", uploaded.filename)
